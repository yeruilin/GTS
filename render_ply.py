...
def create_renders(args):
    ...
    mask=(gaussians.get_colour[:,0]>3e-3).squeeze()

    gaussians.colours=gaussians.colours[mask]
    # gaussians.opacities=gaussians.coefficients[mask]
    gaussians.opacities=gaussians.opacities[mask].unsqueeze(1)
    
    # gaussians.scales=gaussians.scales[mask]
    # print("scaling:",torch.max(gaussians.get_scaling))
    gaussians.scales=torch.log(torch.ones(gaussians.opacities.shape,device=args.device,dtype=torch.float32)*0.01)
...
def get_args():
    ...
    parser.add_argument(
        "--data_path", default="./temp/result500.ply", type=str,
