...
class Scene:
    ...
    def invert_cov_2D(cov_2D: torch.Tensor):
        ...
        Args:
            cov_2D          :   A torch.Tensor of shape (..., 2, 2)

        Returns:
            cov_2D_inverse  :   A torch.Tensor of shape (..., 2, 2)
        """
    ...
    # ================= 向量化 overlap 检测 =================
    def _compute_overlap_mask(self, means_2D, cov_2D, img_size, tile_size):
        """
        参数
        ----
        means_2D : (N, 2)
        ...
        返回
        ----
        overlap  : (N, tiles_y, tiles_x)  bool
        """
        N, (W, H) = means_2D.shape[0], img_size
        tiles_x = (W + tile_size - 1) // tile_size
        tiles_y = (H + tile_size - 1) // tile_size

        # 1) 估算半径：3 * sqrt(max eigenvalue)
        # eig_val = torch.linalg.eigvalsh(cov_2D)          # (N, 2)
        # radius = 3.0 * torch.sqrt(eig_val[:, 1])            # (N,)
        radius = 3.0 *torch.sqrt(cov_2D[:,0:1,0]) # (N,)，只取对角线元素，速度更快

        # 2) Gaussian AABB
        xy_min = means_2D - radius             # (N, 2)
        xy_max = means_2D + radius             # (N, 2)

        # 3) 所有 tile 的左上角
        tx = torch.arange(tiles_x, device=means_2D.device) * tile_size   # (tiles_x,)
        ty = torch.arange(tiles_y, device=means_2D.device) * tile_size   # (tiles_y,)
        # 右下角
        tx_max = tx + tile_size
        ty_max = ty + tile_size

        # 4) 广播求重叠  (N, 1) vs (tiles_x,)  -> (N, tiles_x)
        overlap_x = (xy_min[:, 0:1] < tx_max) & (xy_max[:, 0:1] > tx)   # (N, tiles_x)
        overlap_y = (xy_min[:, 1:2] < ty_max) & (xy_max[:, 1:2] > ty)   # (N, tiles_y)

        # 5) 笛卡尔积： (N, tiles_y, tiles_x)
        overlap = overlap_y.unsqueeze(2) & overlap_x.unsqueeze(1)
        return overlap        # (N, tiles_y, tiles_x)
    ...
    def splat_tiled(
        ...
        opacities: torch.Tensor, img_size: Tuple = (256, 256),
        start_transmittance: Optional[torch.Tensor] = None,
        no_grad=False, tile_size: int = 16
    ):
        """
        Tile-based Gaussian splatting implementation.
        ...
        Args:
            ...
            opacities               :   A torch.Tensor of shape (N,1) with the opacity of each Gaussian.
            img_size                :   The (width, height) of the image.
            start_transmittance     :   Initial transmittance value.
        ...
        Returns:
            ...
            depth                   :   A torch.Tensor of shape (H, W, 1) with the rendered depth map.
            mask                    :   A torch.Tensor of shape (H, W, 1) with the rendered silhouette map.
            final_transmittance     :   A torch.Tensor of shape (1, H, W) representing the transmittance.
        """
        N = means_3D.shape[0]
        W, H = img_size
        ...
        image = torch.zeros((H, W, 3), device=means_3D.device)
        depth = torch.zeros((H, W, 1), device=means_3D.device)
        mask = torch.zeros((H, W, 1), device=means_3D.device)
        final_transmittance = torch.ones((1, H, W), device=means_3D.device)
        
        # 一次算完所有 tile 的 overlap
        overlap = self._compute_overlap_mask(means_2D, cov_2D, img_size, tile_size)
        ...
        for ty in range(tiles_y):
            for tx in range(tiles_x):
                ...
                # Find Gaussians that overlap with this tile
                tile_mask = overlap[:, ty, tx]
                
                # Get indices of Gaussians that overlap with this tile
                tile_indices = torch.where(tile_mask)[0]
                
                if len(tile_indices) == 0:
                    continue  # Skip empty tiles
                
                # Extract tile-specific data
                tile_means_2D = means_2D[tile_indices]
                tile_cov_2D = cov_2D[tile_indices]
                tile_colours = colours[tile_indices]
                tile_opacities = opacities[tile_indices]
                tile_z_vals = z_vals[tile_indices]
                
                # Adjust 2D means to be relative to tile origin
                tile_means_2D_adjusted = tile_means_2D.clone()
                tile_means_2D_adjusted[:, 0] -= x_start
                tile_means_2D_adjusted[:, 1] -= y_start
                
                # Compute alphas for this tile
                tile_alphas = self.compute_alphas(
                    tile_opacities, 
                    tile_means_2D_adjusted, 
                    tile_cov_2D, 
                    (tile_width, tile_height)
                )  # (N_tile, tile_height, tile_width)
                tile_alphas=tile_alphas.reshape(-1,tile_height,tile_width)
                
                # Compute transmittance for this tile
                tile_transmittance = self.compute_transmittance(
                    tile_alphas, 
                    start_transmittance
                )  # (N_tile, tile_height, tile_width)
                
                # Prepare for broadcasting
                tile_z_vals = tile_z_vals[:, None, None, None]  # (N_tile, 1, 1, 1)
                tile_alphas = tile_alphas[..., None]  # (N_tile, tile_height, tile_width, 1)
                tile_colours = tile_colours[:, None, None, :]  # (N_tile, 1, 1, 3)
                tile_transmittance = tile_transmittance[..., None]  # (N_tile, tile_height, tile_width, 1)
                
                # Compute tile contributions
                tile_image = torch.sum(tile_colours * tile_alphas * tile_transmittance, dim=0)
                tile_depth = torch.sum(tile_z_vals * tile_alphas * tile_transmittance, dim=0)
                tile_mask_contrib = torch.sum(tile_alphas * tile_transmittance, dim=0)
                
                # Place tile results in full image
                image[y_start:y_end, x_start:x_end, :] = tile_image
                depth[y_start:y_end, x_start:x_end, :] = tile_depth
                mask[y_start:y_end, x_start:x_end, :] = tile_mask_contrib
                
                # Update transmittance for next iteration
                if start_transmittance is not None:
                    final_transmittance[0, y_start:y_end, x_start:x_end] = tile_transmittance[-1, ..., 0]
    ...
    def render(
        ...
        if num_mini_batches == 1:
            ...
            image, depth, mask,_ = self.splat_tiled(
            ...
            # 使用 tile-based splatting 显存占用降低2-3个数量级
        ...
        # the implementation more memory efficient but at the same time makes it slower.
            ...
            for b_idx in range(num_mini_batches):
                ...
                # Get image, depth and mask via splatting
                image_, depth_, mask_, start_transmittance = self.splat_tiled(
    ...
    def nlos_splat_tiled(self,means_3D,colours,opacities,scales,quats,camera,img_size,tile_size: int = 16):
        """
        Tile-based Gaussian splatting implementation.
        ...
        Returns:
            intensity               :   A torch.Tensor of shape (N,)
        """
        N = means_3D.shape[0]
        W, H = img_size
        ...
        # Initialize output tensors
        intensity = torch.zeros((N,), device=means_3D.device)
        ...
        for ty in range(tiles_y):
            for tx in range(tiles_x):
                ...
                # Compute transmittance for this tile
                tile_transmittance = self.compute_transmittance(tile_alphas) # (N_tile, tile_height*tile_width)
                
                # Compute tile contributions
                tile_intensity = torch.sum(tile_colours * tile_alphas * tile_transmittance, dim=1) # (N_tile,)

                intensity[tile_indices] += tile_intensity
    ...
    def render_conf_hist(self,camera,bin_resolution,num_bins,t0=0,decay=4,per_splat=-1,img_size=(64,64)):
        ...
        # In this case we can directly splat all gaussians onto the image
        if per_splat == -1:
            intensity=self.nlos_splat_tiled(means_3D,colours,opacities,scales,quats,camera,img_size)
            ...
            for i in range(num_mini_batches):
                ...
                intensity_=self.nlos_splat_tiled(means_3D,colours,opacities,scales,quats,camera,img_size,b_idx)
        ...
        coeff=self.gaussians.get_coefficient # (N,1)
