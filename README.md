...
对于非共焦数据，要利用好非共焦两条边的几何关系。激光器对准p0点，面阵探测器在p位置接收回波。因为p0和p能看到的片元是不同的，因此渲染瞬态图只需要先找到p0能看到的片元，然后其他片元都被mask。然后在p的视角对这些片元渲染深度图，用camera的unproject_points函数投影出三维点云，算出v到p0的距离，这样就得到了两段距离。由此可以算出histogram。

当然这里有个问题，如果要不断优化，那么能看见的片元是会改变的；另外有些片元始终被mask，导致无法被优化。优化起来实际上是比较难的。

<p align="center">
  <img src="results/非共焦渲染.png" alt="non-confocal" width=50%/>
</p>
