...
def create_renders(args):
    ...
    z=-2.0 # 扫描平面深度
    laser_point=(0,0,z) # 非共焦激光点位置
    laser_point_tensor=torch.Tensor(laser_point).view(1,3).to(args.device)
    ...
    with torch.no_grad():
        ...
        for i in range(img_size[0]):
            for j in range(img_size[1]):
                ...
                hist,_,_ = scene.render_nonconf_hist(current_camera,laser_point_tensor,args.bin_resolution,args.bin,
                                              args.gaussians_per_splat,img_size,bg_colour,no_grad=True)
                transient_map[i,j,:]=hist.detach().cpu().numpy()
                savemat("temp/signal.mat",{"data":hist.detach().cpu().numpy()})
                exit()

        # 将瞬态图保存为.mat文件
        savemat("temp/nonconfocal_cow.mat",{"data":transient_map,"bin_resolution":args.bin_resolution/3e8,"width":args.width})
