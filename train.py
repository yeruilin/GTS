...
def setup_optimizer(gaussians):
    ...
    # optimizer = torch.optim.Adam(parameters, lr=1e-4, eps=1e-15)
...
def run_training(args):
    ...
    viz_cameras = [train_dataset[i][1].to(args.device) for i in viz_idxs]
    ...
    for itr in range(args.num_itrs):
        ...
        gt_img = gt_img[0].to(args.device)
        camera = camera[0].to(args.device)
        if gt_mask is not None:
            gt_mask = gt_mask[0].to(args.device)
        ...
        pred_img, depth, mask,_,_= scene.render(camera,args.gaussians_per_splat,(gt_img.shape[0],gt_img.shape[1]),bg_colour)

        # Compute loss
        ### YOUR CODE HERE ###
        # HINT: A simple standard loss function should work.
        loss = torch.mean((pred_img-gt_img).abs()) # L1 Loss
    ...
    for viz_data in tqdm(viz_loader, desc="Creating Visualization"):
        ...
        with torch.no_grad():
            ...
            # HINT: Get per_splat from args.gaussians_per_splat
            # HINT: camera is available above
            pred_img, depth, mask,_,_ = scene.render(camera,args.gaussians_per_splat,(gt_img.shape[0],gt_img.shape[1]),bg_colour,no_grad=True)
